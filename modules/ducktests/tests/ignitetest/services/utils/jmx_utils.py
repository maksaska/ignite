...
def metric_registry_pattern(group, name):
    ...
    Two details of that naming decide what the pattern has to look like. The name is quoted
    only when it is not purely alphanumeric - ``name="my-cache"`` but ``name=myCache`` - hence
    the optional quotes. And the properties of an MBean name are ordered alphabetically, so
    the group always comes before the name but never right next to it.
    ...
    """
    return f'.*group={group},.*name="?{name}"?'
